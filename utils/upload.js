...
const fileFilter = (req, file, cb) => {
  ...
  if (mimetype && extname) {
    ...
    cb('Erreur: Voici la liste de type de fichiers supportés - ' + filetypes);
  }
};

// Configure Multer for Posts files
...
// Configure Multer for Profiles files
...
// Configure Multer for Pages files
export const uploadStory = multer({
  storage: multerS3({
    ...
    key: (req, file, cb) => {
      const imageName = crypto.randomBytes(32).toString('hex');
      const fileExtension = file.originalname.split('.').pop();
      const finalName = 'story/' + imageName + '.' + fileExtension;
...
// Configure Multer for Pages files
export const uploadPage = multer({
  storage: multerS3({
    s3: s3Client,
    bucket: process.env.SPACES_BUCKET,
    acl: "public-read",
    ...
    key: (req, file, cb) => {
      const imageName = crypto.randomBytes(32).toString('hex');
      const fileExtension = file.originalname.split('.').pop();
      const finalName = 'page/' + imageName + '.' + fileExtension;
      cb(null, finalName);
    },
    contentType: multerS3.AUTO_CONTENT_TYPE,
  }),
...
// Configure Multer for Messages files
